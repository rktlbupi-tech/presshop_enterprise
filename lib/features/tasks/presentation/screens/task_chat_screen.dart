...
class _TaskChatScreenState extends State<TaskChatScreen> {
  ...
  Future<void> _loadConversation() async {
    ...
    try {
      final resp = await _apiClient.get('chat-v2/enterprise/tasks/$taskId/conversation');
      final d = resp.data;
      if (d['success'] == true && d['data'] != null) {
        ...
        for (final m in members) {
          ...
          if (id.isNotEmpty) _membersMap[id] = Map<String, dynamic>.from(m as Map);
        }
      }
    } catch (_) {}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          'Manage Task',
          style: TextStyle(color: Colors.black, fontWeight: FontWeight.w800, fontSize: 20, fontFamily: 'AirbnbCereal'),
        ),
        centerTitle: false,
