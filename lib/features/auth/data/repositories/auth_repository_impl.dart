...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<(UserEntity?, Failure?)> signup(String fullName, String email, String password) async {
    try {
  ...
  Future<void> _saveSession(UserEntity user) async {
    ...
    if (user.phone != null) await _prefs.setString('user_phone', user.phone!);
    if (user.profileImage != null) await _prefs.setString('user_avatar', user.profileImage!);
    if (user.role != null) await _prefs.setString('user_role', user.role!);
    if (user.companyName != null) await _prefs.setString('company_name', user.companyName!);
    if (user.companyLogo != null) await _prefs.setString('company_logo', user.companyLogo!);
  }
}
