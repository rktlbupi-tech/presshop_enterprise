...
class AttendanceBloc extends Bloc<AttendanceEvent, AttendanceState> {
  ...
  Future<void> _onFetch(FetchAttendanceLog e, Emitter<AttendanceState> emit) async {
    ...
    for (int i = 0; i < 15; i++) {
      ...
      DateTime checkInTime = DateTime(logDate.year, logDate.month, logDate.day, 9, 15);
      DateTime? checkOutTime = DateTime(logDate.year, logDate.month, logDate.day, 17, 45);

      if (i == 2) {
        status = 'late';
        workedHours = 7.0;
        checkInTime = DateTime(logDate.year, logDate.month, logDate.day, 10, 45);
      } else if (i == 5) {
        status = 'absent';
        workedHours = 0.0;
        ...
        status = 'late';
        workedHours = 7.5;
        checkInTime = DateTime(logDate.year, logDate.month, logDate.day, 10, 15);
      }

      dummyLogs.add(AttendanceLogEntity(
        id: 'dummy_$i',
        date: logDate,
  ...
  Future<void> _onCheckIn(CheckInRequested e, Emitter<AttendanceState> emit) async {
    ...
    _isCheckedIn = true;
    emit(const AttendanceActionSuccess('Checked in successfully!', isCheckedIn: true));
  }
  ...
  Future<void> _onCheckOut(CheckOutRequested e, Emitter<AttendanceState> emit) async {
    ...
    await Future.delayed(const Duration(milliseconds: 400));
    _isCheckedIn = false;
    emit(const AttendanceActionSuccess('Checked out successfully!', isCheckedIn: false));
  }
}
